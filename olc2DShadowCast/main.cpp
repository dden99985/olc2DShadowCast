...
private:
    ...
    void ConvertTileMaptoPolyMap(int sx, int sy, int w, int h, float fBlockWidth, int pitch)
    {
        ...
        float borderLeft = 1;
        float borderRight = ((float)nWorldWidth * fBlockWidth) -1;
        float borderTop = fBlockWidth + 1;
        float borderBottom = ((float)nWorldHeight * fBlockWidth) -1;
        vecEdges.push_back({borderLeft, borderTop, borderRight, borderTop});         // Top
        vecEdges.push_back({borderLeft, borderTop, borderLeft, borderBottom});       // Left
        vecEdges.push_back({borderLeft, borderBottom, borderRight, borderBottom});   // Bottom
        vecEdges.push_back({borderRight, borderTop, borderRight, borderBottom});     // Right
        ...
        for(auto &e1 : vecEdges)
        {
            ...
            for(int i = 0; i < 2; i++)
            {
                ...
                for(int j = 0; j < 3; j++)
                {
                    ...
                    // Check for ray intersection with all edges to see if one is closer
                    for(auto &e2 : vecEdges)
                    {
                        ...
                        // Skip rays that are colinear with each other
                        if(fabs(sdx - rdx) > 0.0f && fabs(sdy - rdy) > 0.0f)
                        {
                            // t2 is the normalized distance from start of the line segment to the intersect point
                            float t2 = (rdx * (e2.sy - oy) + (rdy * (ox - e2.sx))) / (sdx * rdy - sdy * rdx);
                            // t1 is normalized distance from source ray to the intersect point
                            float t1 = (e2.sx + sdx * t2 - ox) / rdx;
                            
                            // There seems to be a quirk in t1.  The y axis is ignored.  When ox and one of e1's xs are equal,
                            // t1 = 0 and a visibility point radius distance from ox,oy gets created, passing through any
                            // edges in the way.  I can't figure out how to include y in the formula, but the following hack
                            // seems to fix it.
                            if (t1 == 0 && t2 >= 0 && t2 <= 1.0f)
                            {
                                // Base t1 off y instead of x
                                t1 = (e2.sy + sdy * t2 - oy) / rdy;
                            }

                            // If intersect point exists along ray, and along line segment
                            // then intersect point is valid
                            if (t1 > 0 && t2 >= 0 && t2 <= 1.0f)
                            {
...
public:
    ...
    bool OnUserUpdate(float fElapsedTime) override
    {
        ...
        // Clear up here to allow drawing for debug
